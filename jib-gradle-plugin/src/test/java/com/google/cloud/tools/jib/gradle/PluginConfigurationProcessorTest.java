...
import static com.google.cloud.tools.jib.gradle.PluginConfigurationProcessor.configureContainerizer;
...
public class PluginConfigurationProcessorTest {
    ...
    private static BuildConfiguration getBuildConfiguration(JibContainerBuilder jibContainerBuilder,
                                                            JibExtension mockJibExtension,
                                                            GradleProjectProperties mockProjectProperties
                                                            )
            throws InvalidImageReferenceException, IOException, CacheDirectoryCreationException {
        Containerizer containerizer = Containerizer.to(RegistryImage.named("ignored"));
        configureContainerizer(containerizer, mockJibExtension, mockProjectProperties);
    ...
    @Mock
    private JibExtension mockJibExtension;
    @Mock
    private BaseImageParameters mockBaseImageParameters;
    @Mock
    private ContainerParameters mockContainerParameters;
    @Mock
    private DockerClientParameters mockDockerClientParameters;
    @Mock
    private GradleProjectProperties mockProjectProperties;
    ...
    public void setUp() {
        ...
        Mockito.when(mockJibExtension.getDockerClient()).thenReturn(mockDockerClientParameters);
        Mockito.when(mockDockerClientParameters.getEnvironment()).thenReturn(Collections.emptyMap());
        Mockito.when(mockDockerClientParameters.getExecutable()).thenReturn(Paths.get("/path/to/docker"));
        ...
        Mockito.when(mockJibExtension.getTo()).thenReturn(targetImageParameters);
        Mockito.when(targetImageParameters.getTags()).thenReturn(Collections.emptySet());
    }
    ...
    public void testPluginConfigurationProcessor_defaults()
        ...
        BuildConfiguration buildConfiguration =
                getBuildConfiguration(processor.getJibContainerBuilder(), mockJibExtension, mockProjectProperties);
    ...
    @Test
    public void testDockerClientEnvironmentSetting()
            throws InvalidImageReferenceException, IOException, CacheDirectoryCreationException {
        Map<String, String> dockerClientEnvMap = new HashMap<String, String>();
        dockerClientEnvMap.put("PAR1", "VAL1");
        dockerClientEnvMap.put("PAR2", "VAL2");

        Mockito.when(mockDockerClientParameters.getEnvironment()).thenReturn(dockerClientEnvMap);
        ...
        Assert.assertEquals(
                dockerClientEnvMap,
                buildConfiguration.getDockerClientConfiguration().getEnvironmentMap());
        Mockito.verifyZeroInteractions(mockLogger);
    }

    @Test
    public void testDockerClientPathSetting()
            throws InvalidImageReferenceException, IOException, CacheDirectoryCreationException {
        Path path = Paths.get("/path/to/docker");

        Mockito.when(mockDockerClientParameters.getExecutable()).thenReturn(path);
        ...
        Assert.assertEquals(
                path,
                buildConfiguration.getDockerClientConfiguration().getExecutable());
        Mockito.verifyZeroInteractions(mockLogger);
    }
